...
from .configuration_qwen3_vl import PVMQwen3VLConfig, PVMQwen3VLTextConfig, Qwen3VLVisionConfig
...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: PVMQwen3VLTextConfig, device=None):
        super().__init__()
...
class PVMSimpleRMSNorm(nn.Module):
    """
    Affine-less RMSNorm. 
...
class Qwen3VLTextAttention(nn.Module):
    ...
    def __init__(self, config: PVMQwen3VLTextConfig, layer_idx: int):
        super().__init__()
        self.config = config
...
class PVMCrossAttention(nn.Module):
    """
    PVM Cross Attention: Mimics the Qwen3VLTextAttention structure but operates
    within the reduced 'pvm_hidden_size' dimension.
    ...
    3. Dimensionality: Operates in the bottleneck dimension (pvm_hidden_size) while preserving
    ...
    def __init__(self, config: PVMQwen3VLTextConfig, layer_idx: int):
        ...
        self.hidden_size = config.pvm_hidden_size
        ...
        if self.hidden_size % self.head_dim != 0:
            raise ValueError(
                f"config.pvm_hidden_size ({self.hidden_size}) must be divisible by head_dim ({self.head_dim})"
            )
        ...
        # --- 4. Projection Layers ---
        # Projections operate strictly within the reduced 'pvm_hidden_size'.
        self.q_proj = nn.Linear(
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,        # Query (Text) in pvm_hidden_size
        visual_context: torch.Tensor,       # Key/Value (Vision) in pvm_hidden_size
...
class PVMMLP(nn.Module):
    """
    PVM MLP: A SwiGLU MLP adapted for the bottleneck dimension.
    """
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class PVMMemoryBlock(nn.Module):
    """
    PVM Memory Block (Bottleneck Adapter Architecture).
    ...
    in a compressed latent space ('pvm_hidden_size').
    """
    ...
    def __init__(self, config, layer_idx):
        ...
        self.pvm_dim = config.pvm_hidden_size  # The bottleneck dimension (e.g., 1024)
        
        # 2. Down-Projection Layers (Compression)
        # Project Text and Vision features from original dimension to bottleneck dimension.
        self.text_reducer = nn.Linear(self.orig_hidden_size, self.pvm_dim, bias=False)
        self.vision_reducer = nn.Linear(self.orig_hidden_size, self.pvm_dim, bias=False)
        
        # 3. Up-Projection Layer (Restoration)
        # Project features back to the original dimension for residual addition.
        self.output_expander = nn.Linear(self.pvm_dim, self.orig_hidden_size, bias=False)

        # 4. Internal Components (Bottleneck Space)
        # Norms and Core layers operate in 'pvm_dim'.
        self.input_norm = Qwen3VLTextRMSNorm(self.pvm_dim, eps=config.rms_norm_eps)
        self.post_attn_norm = Qwen3VLTextRMSNorm(self.pvm_dim, eps=config.rms_norm_eps)
        
        # These modules automatically utilize config.pvm_hidden_size internally
        self.cross_attn = PVMCrossAttention(config, layer_idx)
        self.mlp = PVMMLP(config)

        self.output_norm = PVMSimpleRMSNorm(eps=config.rms_norm_eps)
...
class Qwen3VLTextDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: PVMQwen3VLTextConfig, layer_idx: int):
        ...
        # --- PVM MODIFICATION ---
        self.pvm_block = None
        pvm_layers = getattr(config, "pvm_layers", None)

        if pvm_layers is not None and layer_idx in pvm_layers:
            self.pvm_block = PVMMemoryBlock(config, layer_idx)
        # -------------------------
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        # --- 🚨 PVM ARGUMENTS 🚨 ---
        pvm_visual_context: Optional[torch.Tensor] = None,
        pvm_visual_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # --- 🚨 PVM INJECTION START 🚨 ---
        if self.pvm_block is not None and pvm_visual_context is not None:
            pvm_update = self.pvm_block(
                hidden_states=residual, 
                visual_context=pvm_visual_context,
                visual_mask=pvm_visual_mask,
                query_is_visual_mask=visual_pos_masks # Pass the mask here
            )
            hidden_states = hidden_states + pvm_update
        # --- 🚨 PVM INJECTION END 🚨 ---
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    config: PVMQwen3VLTextConfig
    _no_split_modules = ["Qwen3VLTextDecoderLayer"]

    def __init__(self, config: PVMQwen3VLTextConfig):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        pvm_visual_context (`torch.Tensor` of shape `(batch_size, vis_seqlen, hidden_size)`, *optional*):
            The visual context for PVM cross-attention.
        pvm_visual_mask (`torch.Tensor` of shape `(batch_size, 1, 1, vis_seqlen)`, *optional*):
            The attention mask for PVM cross-attention
        """
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            layer_outputs = decoder_layer(
                ...
                position_embeddings=position_embeddings,
                # --- 🚨  PVM  🚨 ---
                pvm_visual_context=pvm_visual_context,
                pvm_visual_mask=pvm_visual_mask,
                # Pass visual_pos_masks to ALL layers for PVM silencing
...
@auto_docstring
class PVMQwen3VLModel(Qwen3VLPreTrainedModel):
    base_model_prefix = ""
    ...
    def __init__(self, config):
        ...
        # --- 🚨 PVM STATE CACHE 🚨 ---
        self.pvm_visual_context_cache = None
        self.pvm_visual_mask_cache = None
        # -----------------------------
    ...
    def forward(
        ...
        video_grid_thw: Optional[torch.LongTensor] = None,
        cache_position: Optional[torch.LongTensor] = None,
        # --- 🚨 PVM CACHE ARGUMENT START 🚨 ---
        cached_pvm_visual_context: Optional[torch.Tensor] = None,
        cached_pvm_visual_mask: Optional[torch.Tensor] = None,
        # --- 🚨 PVM CACHE ARGUMENT END 🚨 ---
        **kwargs: Unpack[TransformersKwargs],
        ...
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
            The temporal, height and width of feature shape of each video in LLM.
        cached_pvm_visual_context (`torch.Tensor` of shape `(batch_size, vis_seqlen, hidden_size)`, *optional*):
            The cached visual context for PVM cross-attention during generation.
        cached_pvm_visual_mask (`torch.Tensor` of shape `(batch_size, 1, 1, vis_seqlen)`, *optional*):
            The cached attention mask for PVM cross-attention during generation.
        """
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
        ...
        pvm_visual_context = None
        pvm_visual_mask = None

        if cached_pvm_visual_context is not None:
            pvm_visual_context = cached_pvm_visual_context
            pvm_visual_mask = cached_pvm_visual_mask
            ...
            pvm_visual_context = pad_sequence(extracted_visuals, batch_first=True)
            
            lengths = [x.shape[0] for x in extracted_visuals]
            max_len = pvm_visual_context.shape[1]
            pvm_visual_mask = torch.full(
                (bsz, 1, 1, max_len), 
            ...
            for i, l in enumerate(lengths):
                if l > 0:
                    pvm_visual_mask[i, ..., :l] = 0.0
            
            self.pvm_visual_context_cache = pvm_visual_context
            self.pvm_visual_mask_cache = pvm_visual_mask
            ...
            pvm_visual_context = self.pvm_visual_context_cache
            pvm_visual_mask = self.pvm_visual_mask_cache
...
class PVMQwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    _checkpoint_conversion_mapping = {}
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = PVMQwen3VLModel(config)
...
__all__ = [
    "Qwen3VLVisionModel",
    "PVMQwen3VLForConditionalGeneration",
    "PVMQwen3VLModel",
